...
for TIP in {1..36}                         ; do
   for SES in   12                         ; do                                        #Racunam spread samo JFM

# (1/3) dobiti ensemble srednjak po odabranim godinama
      for MOD in {1..15}               ; do
      INPUT=./2014_pr/FILES_RCM/${VAR}_MOD_${MOD}_SM_${SEAStxt[${SES}]}_SMS_2014.nc    #MOD by MOD, YEAR by YEAR
      ...
      #-----------------------------------------
      #-----------------------------------------
# (2/3) dobiti anomalije za svaki model po odabranim godinama u odnosu na (1/3)
	for MOD in {1..15}                 ; do 
	        INPUT1=./2014_pr/FILES_RCM/${VAR}_MOD_${MOD}_SM_${SEAStxt[${SES}]}_SMS_2014.nc 
		INPUT2=temp_ENSAVG.nc
		OUTPUT=${VAR}_MOD_${MOD}_SM_${SEAStxt[${SES}]}_TIP${TIP}_anomalySquared_SMS_2016.nc  
        	rm -rfv temp.nc
		cdo selyear,${GODINE[${TIP}]}  ${INPUT1} temp.nc
		cdo setday,15 -settime,12:00:00 -timavg -pow,2 -sub temp.nc ${INPUT2} ${OUTPUT}
	        rm -rfv temp.nc
        done #od MOD

# (3/3) dobiti ensemble srednjak od (2/3)
	rm  -rfv temp.nc
	OUTPUT=${VAR}_MOD_ENSAVG_SM_${SEAStxt[${SES}]}_TIP${TIP}_spread6_SMS_2016.nc
	cdo ensavg ${VAR}_MOD_*_SM_${SEAStxt[${SES}]}_TIP${TIP}_anomalySquared_SMS_2016.nc  temp.nc 
        cdo sqrt temp.nc ${OUTPUT}
 	rm -rfv temp.nc
        rm -rfv temp_ENSAVG.nc temp_MOD_1-15.nc 
        rm -rfv temp_MOD?.nc temp_MOD??.nc
	rm -rfv ${VAR}_MOD_*_SM_${SEAStxt[${SES}]}_TIP${TIP}_anomalySquared_SMS_2016.nc
