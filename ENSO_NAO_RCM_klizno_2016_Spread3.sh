...
#-----------------
     DIRCRU=/home/guettler/bonus_disk/MODELS/ENSEMBLES/DATA_CRU
     DIRWRK=/home/guettler/bonus_disk/MODELS/ENSEMBLES/DIR_ANALIZA_ENSO_NAO

# STEP 1: OK
#    for MOD in $(seq 1 ${NMODS}) ; do
#        INPUT=${DIRCRU}/${VAR}_CRU/${FILE[${MOD}]}
#       OUTPUT=${DIRWRK}/running.nc
#        cdo -r -f nc -b 32 -setday,15 -runmean,3 -seldate,1961-02-01,2000-12-31 ${INPUT} ${OUTPUT}
#        for SES in {1..12} ; do
#             INPUT=${DIRWRK}/running.nc
#            OUTPUT=${DIRWRK}/${VAR}_MOD_${MOD}_SM_${SEAStxt[${SES}]}_SMS_2014.nc
#            cdo -r -selmon,${SETmontxt[${SES}]}  ${INPUT} ${OUTPUT}
#        done #SES
#        rm ${DIRWRK}/running.nc
#    done #MOD
# STEP 2: OK
#    for SES in {1..12}    ; do
#   if [ ${NMODS} == 15 ] ; then
...
#        OUTPUT1=${VAR}_MOD_ENSAVG_TIMAVG_SM_${SEAStxt[${SES}]}_SMS_2014.nc #visegodisnji srednjak po SVIM godinama
#        OUTPUT2=${VAR}_MOD_ENSAVG_TIMAVG_TIP${TIP}_SM_${SEAStxt[${SES}]}_SMS_2014.nc #visegodisnji srednjak po ODBRANIM godinama
#
...
#        INPUT=${VAR}_MOD_ENSAVG_SM_${SEAStxt[${SES}]}_SMS_2014.nc
#        OUTPUT1=${VAR}_MOD_ENSAVG_TIMAVG_SM_${SEAStxt[${SES}]}_SMS_2014.nc #visegodisnji srednjak po SVIM godinama
#        OUTPUT2=${VAR}_MOD_ENSAVG_TIMAVG_TIP${TIP}_SM_${SEAStxt[${SES}]}_SMS_2014.nc #visegodisnji srednjka po ODABRANIM godinama
#
#        cdo timavg                               ${INPUT} ${OUTPUT1}
...
for TIP in {1..36}                     ; do
   for SES in 12                       ; do
    INPUT1=./2016_pr/FILES_RCM/${VAR}_MOD_ENSAVG_SM_${SEAStxt[${SES}]}_SMS_2014.nc #ENSEMBLE MEAN za GODINU po GODINU u grupi
    OUTPUT=${VAR}_MOD_ENSAVG_SM_${SEAStxt[${SES}]}_TIP${TIP}_anomalySquared_SMS_2016.nc  
       	rm -rfv temp.nc
	cdo timavg -selyear,${GODINE[${TIP}]}  ${INPUT1} temp_timavg.nc
	cdo         selyear,${GODINE[${TIP}]}  ${INPUT1} temp.nc
	cdo timavg -pow,2 -sub temp.nc temp_timavg.nc    ${OUTPUT}
        rm -rfv temp.nc temp_timavg.nc

#2016-08-30    INPUT1=${VAR}_MOD_*_SM_${SEAStxt[${SES}]}_TIP${TIP}_anomalySquared_SMS_2016.nc
     INPUT1=${VAR}_MOD_ENSAVG_SM_${SEAStxt[${SES}]}_TIP${TIP}_anomalySquared_SMS_2016.nc
     OUTPUT=${VAR}_MOD_ENSAVG_SM_${SEAStxt[${SES}]}_TIP${TIP}_spread3_SMS_2016.nc
#2016-08-30	cdo ensavg       ${INPUT1}  temp.nc 
#2016-08-30     cdo sqrt temp.nc ${OUTPUT}
        cdo sqrt ${INPUT1} ${OUTPUT}
	rm -rfv temp.nc
	rm -rfv ${VAR}_MOD_ENSAVG_SM_${SEAStxt[${SES}]}_TIP${TIP}_anomalySquared_SMS_2016.nc
   done #od SES
done #od TIP
#-----------------
#----------------- STEP 5: preskoceno!
