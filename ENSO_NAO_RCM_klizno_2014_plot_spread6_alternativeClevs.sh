...
function addLines1
     VAR=${VAR}
     SESI=${SESI}
      if (VAR=1);  'set clevs    0.5 1 2 3 4 5 6 7 8'                    ;  endif;
return
EOF
cat > addLines2 << EOF
function addLines2
...
while TIT<=6
FILE=TIT+6*(GGGG-1)
    if (VAR=1); 'sdfopen /home/guettler/bonus_disk/MODELS/ENSEMBLES/DIR_ANALIZA_ENSO_NAO/2016_pr/FILES_RCM/pr_MOD_ENSAVG_SM_'SESItxt.SESI'_TIP'FILE'_spread6_SMS_2016.nc';         endif
    TIT=TIT+1
endwhile
if (VAR=1);
     varic1='pr'
...
if [ ${VAR} == 1 ]; then
FILE=RCM_GROUP${GGG}_${SESItxt[${SESI}]}_pr_spread6_smth9_alternativeClevs
fi
