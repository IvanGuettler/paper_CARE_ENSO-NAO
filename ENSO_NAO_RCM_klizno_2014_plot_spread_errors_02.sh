...
function addLines1
     VAR=${VAR}
     SESI=${SESI}
     if (VAR=1);  'set clevs   0.2 0.3 0.4 0.5 1 1.5 2 2.5 3 ';    endif;
return
EOF
cat > addLines2 << EOF
function addLines2
...
*--------------------------------------------------------------------------------------------------------
    ...
    'sdfopen ./ErrorSpread_'SESItxt.SESI'_vsERA40.nc'
...
say '------------------------------------------------------------------------->'FILE
      ...
      if (VAR=1); 'draw title RCM-ERA40 error spread (mm/day)  'SESItxt.SESI' \ 0.2 0.3 0.4 0.5 1 1.5 2 2.5 3; smoothed'; endif
...
if [ ${VAR} == 1 ]; then
FILE=RCM_${SESItxt[${SESI}]}_pr_errorSpread_vsERA40
fi
