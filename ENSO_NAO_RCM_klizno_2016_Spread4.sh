...
for TIP in {1..36}                     ; do
   for SES in 12                       ; do
                OUTPUT=${VAR}_MOD_ENSAVG_SM_${SEAStxt[${SES}]}_TIP${TIP}_spread4_SMS_2016.nc
      for MOD in {1..15}               ; do
	        INPUT=./2014_pr/FILES_RCM/${VAR}_MOD_${MOD}_SM_${SEAStxt[${SES}]}_SMS_2014.nc       #MODEL by MODEL, YEAR by YEAR
       		rm -rfv temp_MOD${MOD}.nc
		cdo setday,15 -settime,12:00:00 -timavg -selyear,${GODINE[${TIP}]}  ${INPUT} temp_MOD${MOD}.nc
      done #od MOD
        	rm -rfv temp_ENSAVG.nc temp_MOD_1-15.nc
	        cdo ensavg temp_MOD*.nc temp_ENSAVG.nc
                cdo copy   temp_MOD??.nc temp_MOD?.nc temp_MOD_1-15.nc
		cdo sqrt -timavg -pow,2 -sub temp_MOD_1-15.nc temp_ENSAVG.nc  ${OUTPUT}
